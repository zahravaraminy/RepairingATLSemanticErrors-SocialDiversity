...
public class NSGAIIThreeStep extends Algorithm {
  ...
  public static int fitness3=100000;
  ...
  public SolutionSet execute() throws JMException, ClassNotFoundException, Exception,NullPointerException  {
    ...
    DistanceSocialDiversity distance2 = new DistanceSocialDiversity(); //step 4
    //Read the parameters
     ...
     while (evaluations < maxEvaluations) {
      ...
      // Create the solutionSet union of solutionSet and offSpring
    	 ...
    	 for (int k = 0; k < front.size(); k++) {
    		 ...
    		 writer.println(front.get(k).getfitnessthird());
    		 writer.flush();
...
private void AddRuleNametoArrayList(String line,String[] parts, Setting s)
  ...
  private void ReadInputDiff() {
	  
	  for(int i=0; i<4; i++)
	  {
	  BufferedReader bf = null;
	  try {
		bf = new BufferedReader(new FileReader(NSGAIIThreeStep.startingroot+"/varaminz/"+NSGAIIThreeStep.step+"/examples/class2rel/trafo/semanticfaulty/atl"+Class2Rel.atlindex+"/inputDiff"+i+".txt"));
	} catch (FileNotFoundException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
	  ...
	  try {
		while (( line = bf.readLine()) != null)
			...
			String [] arr7= arr[7].trim().split("left");
			...
			String [] arr8= arr[8].trim().split("right");
			...
			if(i==0 )
				Diffsubset0.add( builder.toString());
			else if(i==1)
				Diffsubset1.add( builder.toString());
			else if(i==2)
				Diffsubset2.add( builder.toString());
			else if(i==3)
				Diffsubset3.add( builder.toString());
			}
			else {
				String [] arr7= line.trim().split("resourceURI");
			 if(i==0)
				Diffsubset0.add(arr7[0]);
			  else if(i==1)
				Diffsubset1.add(arr7[0]);
			  else if(i==2)
				Diffsubset2.add(arr7[0]);
			  else if(i==3)
				Diffsubset3.add(arr7[0]);	
			}
	  ...
	  diversityMaxn=Diffsubset0.size()+Diffsubset1.size()+Diffsubset2.size()+Diffsubset3.size();
